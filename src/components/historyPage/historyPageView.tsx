      ...
      {!isPageLoading && (
        <>
          ...
          <div>
            ...
            {!isLoggedIn && <span>Login to see history</span>}
          </div>

          <h1 className="text-3xl mt-8 mb-4">Saved Jobs</h1>
          <div className="w-full flex justify-center">
            {isLoggedIn && (
              <JobListView jobs={savedJobs} loading={isJobsLoading} skills={[]} jobsPerQuery={3} noJobsFound={false} />
            )}
            {isLoggedIn && !isJobsLoading && savedJobs?.length === 0 && (
              <Link href="/">There are no jobs saved, save one after a search</Link>
            )}
            {!isLoggedIn && <span>Login to see saved jobs</span>}
          </div>
        </>
      )}
      <div className="absolute top-1/2 left-1/2">
        <Spinner hidden={!isPageLoading} size="xl" />
      </div>
    </div>
  )
}
...
const SearchesTable = ({ searches }: { searches: Search[] }) => {
  return (
    <Table striped={true}>
      <Table.Head>
        <Table.HeadCell>Seach type</Table.HeadCell>
      ...
      <Table.Body className="divide-y">
        {Children.toArray(
          searches.map((s: Search, index) => (
            <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800" key={index}>
              <Table.Cell className="whitespace-nowrap font-medium text-gray-900 dark:text-white">{s.type}</Table.Cell>
              <Table.Cell>{s.createdAt}</Table.Cell>
              <Table.Cell>{s.params}</Table.Cell>
              <Table.Cell>
                <a href={`/${s.link}`} className="font-medium text-blue-600 hover:underline dark:text-blue-500">
                  View
                </a>
              </Table.Cell>
            </Table.Row>
